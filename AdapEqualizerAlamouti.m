function [y,pArr,w11_Arr,w12_Arr,w21_Arr,w22_Arr,errorArr] = AdapEqualizerAlamouti(x, trainSymbs, SpS, NTaps, Mu_T, Mu_DD, Mu_p, N1, modFormat)
    ...
    % SpS - samples per symbol. Code only works with SpS=1 for now
    ...
    pArr = zeros(OutLength,2); vArr = zeros(OutLength,2); errorArr = zeros(OutLength,2);
    ...
    w11(floor(NTaps/2)+1) = 1;
    w22(floor(NTaps/2)+1) = -1; 
...
%         p4 = p4 + Mu_p*e_e*conj(w22'*conj(u_e(:,i)));
        ...
        % Second phase correction term (which is excluded for now)
